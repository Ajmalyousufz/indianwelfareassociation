...
(function ($) {
	...
	var carousel = function () {
		$('.carousel-cause').owlCarousel({
			...
			touchDrag: true, // Ensures horizontal touch swipe works
			responsive: {
				0: {
					items: 1,
					stagePadding: 0
		...
		let startY;
		$('.carousel-cause').on('touchstart', function (e) {
			startY = e.originalEvent.touches[0].pageY; // Get the initial Y touch position
		});

		$('.carousel-cause').on('touchmove', function (e) {
			let currentY = e.originalEvent.touches[0].pageY;
			let deltaY = Math.abs(currentY - startY);

			if (deltaY > 10) {
				// If vertical movement is detected, allow vertical scroll
				e.stopPropagation();
			}
		});
